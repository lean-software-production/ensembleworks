#!/usr/bin/env bash
#
# livekit-cutover-ash.sh — flip EnsembleWorks from LiveKit Cloud to self-hosted
# LiveKit OSS on this VM. Idempotent + reversible.
#
# Run as ROOT on the VM (htz-useast-ash-001):
#   bash livekit-cutover-ash.sh            # do the cutover (steps 1-4)
#   bash livekit-cutover-ash.sh --rollback # restore LiveKit Cloud + stop the SFU
#   bash livekit-cutover-ash.sh --status   # show current state, change nothing
#
# What it does:
...
#
# Rollback: bash livekit-cutover-ash.sh --rollback
...
# Generated by livekit-cutover-ash.sh. Root-only. Reused on re-runs so the
...
# Written by livekit-cutover-ash.sh. Signaling (7880) bound to loopback, proxied
...
# Self-hosted LiveKit OSS (written by livekit-cutover-ash.sh).
...
cat >&2 <<'VERIFY'
  ...
  If ANYTHING fails, rollback is one command:
    bash livekit-cutover-ash.sh --rollback
  ...
  Status anytime:  bash livekit-cutover-ash.sh --status
VERIFY
