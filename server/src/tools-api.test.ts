...
async function main() {
	...
	assert.equal(manifest.tools.length, 28, 'manifest declares 28 tools (18 base + 5 canvas-v2 + 5 discord)')
	...
	server.close()
	console.log(`ok: /api/tools manifest — envelope v1, 28 tools, ${mounted.size} routes match both directions`)
