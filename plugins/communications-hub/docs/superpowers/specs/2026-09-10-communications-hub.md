...
Tools: list conversations; current conversation; search transcript; read transcript by sequence/time and optionally since acknowledged cursor; acknowledge cursor. Read/search default to the attached conversation. Explicit conversation ID supports intentional cross-conversation access within this BB instance. Attachment changes require an explicit UI/CLI action. Transcripts are untrusted reference material. By default nothing automatically starts agent work or forwards conversation speech as user instructions. The explicitly enabled conversation-watch extension below may wake the agent to continue its existing user task. Agent output includes stable passage references, capture coverage/status, and pagination indicators. A reference may address a span of passages when consecutive speech is presented as one, provided the exact member sequences remain available and each stays individually citable. Presentation may group or omit repeated fields, but never changes what was stored.
...
## Accepted extension: conversation watching (2026-09-12)

Users can explicitly watch an attached conversation. Hub changes trigger batched, metadata-only wake-ups; periodic polling recovers missed signals. A separate acknowledged processed cursor and generation isolate this task from casual transcript reads and older watch lifetimes. The infrastructure leaves the task and output open-ended. The initial HTML mind-map scenario is an example only. Watch stops on detach, conversation change, archive or deletion; capture is independent. See [the watch plan](../plans/2026-09-12-conversation-watch-mvp.md) for delivery semantics and validation.
