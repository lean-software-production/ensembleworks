...
function policy() {
  ...
  if (changed.some(p => p.startsWith('.fabro/lines/'))) throw new Error('Line definitions must remain unchanged');
...
try {
  ...
  if (phase === 'baseline') {
    ...
    if (scope.changed.length) {
      ...
      if (hasStaged.status === 1) execFileSync('git',['-c','user.name=Assembly Lines','-c','user.email=assembly-lines@localhost','commit','-m','fix: repair reproduced bug'],{stdio:'inherit'});
