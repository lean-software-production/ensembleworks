...
Do not use review-change to fix findings. Its delivery outcome `reviewed` says only that the review completed. Do not treat a compiler/setup error as a valid bug reproduction. The bug line checks exit codes mechanically; its planner/reviewer must establish that the recorded failure matches the bug.

All lines use `.fabro-input/work-order.json` and `.fabro-output/delivery.json`, preserving BB origin-thread review. Commands, scope, budgets, environment and models must fit the target project. Node timeouts are 30 minutes; runner elapsed/attempt checks are not a hard process-tree deadline during agent waits.

## Provenance

Recipe edition: 1 (2026-09-13). The assets are maintained in this BB plugin, based on its dogfooded refactor and Ralph runners; review and bug variants add their own delivery guards. They are copied into the project, not linked to upstream HEAD and not overwritten by plugin updates. The existing create-fabro-line refactor asset remains the shared source for that recipe.
