...
function fixture(t,recipes=names){
 ...
 const order=name=>{const o=JSON.parse(readFileSync(join(root,'.fabro/workflows',name,'work-order.example.json'))).workOrder;o.baseSha=sha;o.objective='Verify recipe';return o;};
 const input=o=>write('.fabro-input/work-order.json',JSON.stringify(o));
 const phase=(name,p)=>run(process.execPath,[join(root,'.fabro/workflows',name,'runner.mjs'),p]);
 const delivery=name=>{const d=JSON.parse(readFileSync(join(root,'.fabro-output/delivery.json')));const m=JSON.parse(readFileSync(join(root,'.fabro/workflows',name,'workflow.json')));const v=new Ajv().compile(m.outputSchema);assert.ok(v(d),JSON.stringify(v.errors));return d;};
...
test('all recipes compile, graphs validate, inspection does not execute scripts, overwrite refused',t=>{
 ...
 for(const name of names){const dir=join(f.root,'.fabro/workflows',name);const m=JSON.parse(readFileSync(join(dir,'workflow.json')));const inputs=f.order(name).inputs;if(name==='implement-plan')inputs.planPath='plan.md';if(name==='review-change')inputs.reviewBaseSha=f.sha;if(name==='fix-bug')Object.assign(inputs,{reproductionCommand:'node regression.cjs',expectedFailureExitCode:1,regressionPaths:['regression.cjs']});const validate=new Ajv().compile(m.inputSchema);assert.ok(validate(inputs),JSON.stringify(validate.errors));new Ajv().compile(m.outputSchema);assert.ok(!readFileSync(join(dir,'workflow.fabro'),'utf8').includes('__WORKFLOW_DIR__'));const cli=process.env.FABRO_BIN; if(cli)f.ok(cli,['validate',join(dir,'workflow.fabro')]);}
 assert.notEqual(f.create().status,0);
});
test('reject traversal and symlink destinations',t=>{
 const f=fixture(t,['refactor-code-quality']);
 f.write('config.json',JSON.stringify({recipes:['review-change','refactor-code-quality'],validationCommands:['node --check math.cjs'],qualityCommand:'git diff --check',scope:['math.cjs']}));assert.notEqual(f.create().status,0);assert.ok(!existsSync(join(f.root,'.fabro/workflows/review-change')));
 f.write('config.json',JSON.stringify({recipes:['fix-bug'],validationCommands:['node --check math.cjs'],qualityCommand:'git diff --check',scope:['../outside']}));assert.notEqual(f.create().status,0);assert.ok(!existsSync(join(f.root,'.fabro/workflows/fix-bug')));
