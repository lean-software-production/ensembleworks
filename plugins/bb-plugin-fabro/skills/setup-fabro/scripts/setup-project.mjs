...
const catalog = {
  'refactor-code-quality': { assets: '../../create-fabro-workflow/assets/refactor-workflow', name: 'Improve code quality' },
...
const workflows = join(root,'.fabro','workflows');
await directories(workflows);
if (mode === 'inspect') {
  const markers = [];
  ...
  let existing = []; try { existing = await readdir(workflows); } catch(e) { if(e.code !== 'ENOENT') throw e; }
  console.log(JSON.stringify({ project: root, markers, scripts, existingWorkflows: existing, recipes: Object.keys(catalog), note: 'Suggested commands require inspection; nothing was executed.' },null,2));
} else {
  ...
  for (const name of config.recipes) {
    try { await lstat(join(workflows,name)); throw new Error(`Workflow exists; refusing overwrite: ${name}`); } catch(e) { if(e.code !== 'ENOENT') throw e; }
  }
  const prepared = [];
  for (const name of config.recipes) {
    ...
    const files = {};
    for (const file of ['workflow.json','workflow.fabro','runner.mjs']) files[file] = (await readFile(join(assetPath,file),'utf8')).replaceAll('__WORKFLOW_DIR__',relative);
    const manifest = JSON.parse(files['workflow.json']); manifest.environment = config.environment ?? 'local';
    files['workflow.json'] = JSON.stringify(manifest,null,2)+'\n';
    ...
    files['work-order.example.json'] = JSON.stringify({ requestKey: `${name}-1`, workOrder: {
      title: catalog[name].name, displayTitle: catalog[name].name, objective: 'REPLACE_WITH_CONCRETE_OBJECTIVE', baseSha: 'REPLACE_WITH_FULL_COMMIT_SHA', workflow: `${relative}/workflow.json`, inputs, scope: config.scope,
      constraints: ['Preserve repository instructions and unrelated behavior; do not weaken checks or edit workflow definitions.', 'No merge, push, deployment or external comments.'],
      acceptanceCriteria: ['REPLACE_WITH_TASK_SPECIFIC_CRITERIA', 'Pass configured checks and satisfy the workflow delivery contract.'], maxAttempts: name === 'implement-plan' ? 3 : 2, maxMinutes: name === 'implement-plan' ? 60 : 30, maxChangedFiles: 10,
    }},null,2)+'\n';
  ...
  try {
    for (const {name,files} of prepared) {
      const target = join(workflows,name); await mkdir(target); installed.push(target);
  ...
  console.log(JSON.stringify({created:prepared.map(p => join(workflows,p.name,'workflow.json')), readyToRun:false, next:'Customize task placeholders, validate schemas/graphs and baseline/failure paths, then commit before explicitly submitting.'},null,2));
}
