...
/** Append-only migration statements for the fabro-workflow job store. */
export const JOB_MIGRATIONS = [
  `CREATE TABLE IF NOT EXISTS fabro_jobs (
    id TEXT PRIMARY KEY,
  ...
  `CREATE INDEX IF NOT EXISTS fabro_jobs_thread_cursor
    ON fabro_jobs(thread_id, created_at, id)`,
  `CREATE INDEX IF NOT EXISTS fabro_jobs_pending
    ON fabro_jobs(create_state, observation_state, completion_state, created_at, id)`,
] as const;
...
export class JobStore {
  ...
  create(input: JobInput, now = Date.now()): { job: Job; created: boolean } {
    ...
    const existing = this.#db.prepare("SELECT * FROM fabro_jobs WHERE thread_id=? AND request_key=?").get(input.threadId, input.requestKey) as Row | undefined;
    if (existing) {
    ...
    this.#db.prepare(`INSERT INTO fabro_jobs
  ...
  get(jobId: string): Job {
    id("jobId", jobId);
    const row = this.#db.prepare("SELECT * FROM fabro_jobs WHERE id=?").get(jobId) as Row | undefined;
  ...
  list(options: { threadId?: string; limit?: number; after?: string } = {}): JobPage {
    ...
    const rows = this.#db.prepare(`SELECT * FROM fabro_jobs ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY created_at,id LIMIT ?`).all(...params, limit + 1) as Row[];
  ...
  listPending(limit = 50, after?: string): JobPage {
    ...
    if (cursor) { where.push("(created_at > ? OR (created_at = ? AND id > ?))"); params.push(cursor.createdAt, cursor.createdAt, cursor.id); }
    const rows = this.#db.prepare(`SELECT * FROM fabro_jobs WHERE ${where.join(" AND ")} ORDER BY created_at,id LIMIT ?`).all(...params, checked + 1) as Row[];
  ...
  setPrepared(jobId: string, prepared: { workspacePath: string; workflowVersionId: string; engineUrl?: string | null; sourcePath?: string | null }, now = Date.now()): Job {
    ...
    this.#db.prepare("UPDATE fabro_jobs SET workspace_path=?,workflow_version_id=?,engine_url=coalesce(engine_url,?),source_path=coalesce(source_path,?),updated_at=? WHERE id=?")
  ...
  associateRun(jobId: string, runId: string, now = Date.now()): Job {
    ...
    this.#db.prepare(`UPDATE fabro_jobs SET run_id=?,create_state='confirmed',observation_state=CASE WHEN observation_state='unreachable' THEN 'pending' ELSE observation_state END,updated_at=? WHERE id=?`).run(runId, now, jobId);
    return this.get(jobId);
  }

  recordCreateReconciliation(jobId: string, state: CreateState, createDetail?: string | null, now = Date.now()): Job {
    this.get(jobId); this.#db.prepare("UPDATE fabro_jobs SET create_state=?,create_detail=?,updated_at=? WHERE id=?").run(state, detail(createDetail), now, jobId); return this.get(jobId);
  }
  ...
  recordObservation(jobId: string, state: ObservationState, result?: JsonValue | null, observationDetail?: string | null, resultRevision?: number | null, now = Date.now(), engineStatus?: string | null, connectionError?: string | null): Job {
    ...
    this.#db.prepare(`UPDATE fabro_jobs SET observation_state=?,engine_status=?,connection_error=?,observation_detail=?,result_json=?,result_revision=?,updated_at=? WHERE id=?`).run(
  ...
  recordCompletionDispatch(jobId: string, state: CompletionState, messageId?: string | null, completionDetail?: string | null, now = Date.now()): Job {
    this.get(jobId); this.#db.prepare("UPDATE fabro_jobs SET completion_state=?,completion_message_id=?,completion_detail=?,updated_at=? WHERE id=?").run(state, messageId ?? null, detail(completionDetail), now, jobId); return this.get(jobId);
  }
  ...
  recordAcceptance(jobId: string, verdict: AcceptanceVerdict, reason?: string | null, resultRevision?: number | null, now = Date.now()): Job {
    ...
    this.#db.prepare("UPDATE fabro_jobs SET acceptance_verdict=?,acceptance_reason=?,acceptance_result_revision=?,updated_at=? WHERE id=?").run(verdict, detail(reason), resultRevision ?? null, now, jobId); return this.get(jobId);
  }
