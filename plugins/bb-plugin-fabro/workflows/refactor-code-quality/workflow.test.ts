...
describe.skipIf(!process.env.FABRO_GRAPH_TEST)("live Fabro graph", () => {
  it.each([
    ...
    const run = await client.createRun({ workflow_version_id: version, target: { kind: "folder", path: fixture.cwd }, environment_id: "local", title: "Fabro graph regression", args: { auto_approve: true, labels: { bb_test: "fabro-graph" } } });
...
function repo(overrides: Record<string, unknown> = {}) {
  const cwd = mkdtempSync(join(tmpdir(), "fabro-workflow-runtime-"));
