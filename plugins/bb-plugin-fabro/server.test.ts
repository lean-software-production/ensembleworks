...
function makeHost() {
  const host = createFakePluginHost({
    experimental_callHostRpc: async ({method}) => { if (method === 'validateWorkflow') return {name:'Refactor'}; throw new Error(method); },
...
describe("fabro-workflow submission server", () => {
  it('requires a project workflow and refuses acceptance without a valid delivery contract', async () => {
    const host = makeHost(); await plugin(host.bb);
    const {workflow: _line, ...missingLine} = workOrder;
    await expect(host.harness.behavior.callAgentTool('fabro_workflow_submit', {requestKey:'missing',workOrder:missingLine}, {threadId:'thread-1',projectId:'project-1'})).rejects.toThrow();
    const submitted = toolJson(await host.harness.behavior.callAgentTool('fabro_workflow_submit', {requestKey:'contract',workOrder}, {threadId:'thread-1',projectId:'project-1'}));
    const store = new JobStore(host.bb.storage.database());
    store.recordObservation(submitted.job.id,'succeeded',{workflowContract:{valid:false}},null,1,Date.now(),'succeeded');
    await expect(host.harness.behavior.callAgentTool('fabro_workflow_accept', {jobId:submitted.job.id,resultRevision:1,verdict:'accepted',reason:'looks fine'}, {threadId:'thread-1',projectId:'project-1'})).rejects.toThrow('delivery contract');
    store.recordObservation(submitted.job.id,'succeeded',{workflowContract:{valid:true}},null,1,Date.now(),'succeeded');
    const accepted = toolJson(await host.harness.behavior.callAgentTool('fabro_workflow_accept', {jobId:submitted.job.id,resultRevision:1,verdict:'accepted',reason:'inspected evidence'}, {threadId:'thread-1',projectId:'project-1'}));
    expect(accepted.job.acceptanceVerdict).toBe('accepted');
  });
  it("freezes duplicate submissions to one durable job", async () => {
    const host = makeHost(); await plugin(host.bb);
    const first = toolJson(await host.harness.behavior.callAgentTool("fabro_workflow_submit", { requestKey: "same", workOrder }, { threadId: "thread-1", projectId: "project-1" }));
    const second = toolJson(await host.harness.behavior.callAgentTool("fabro_workflow_submit", { requestKey: "same", workOrder }, { threadId: "thread-1", projectId: "project-1" }));
  ...
  it("rejects a duplicate request key when the frozen work order differs", async () => {
    const host = makeHost(); await plugin(host.bb);
    await host.harness.behavior.callAgentTool("fabro_workflow_submit", { requestKey: "same", workOrder }, { threadId: "thread-1", projectId: "project-1" });
    await expect(host.harness.behavior.callAgentTool("fabro_workflow_submit", { requestKey: "same", workOrder: { ...workOrder, objective: "changed" } }, { threadId: "thread-1", projectId: "project-1" })).rejects.toThrow(/different input/);
  });
  ...
  it("runs the durable host lifecycle through evidence and queues a completion", async () => {
    ...
    const host = createFakePluginHost({
      ...
      experimental_callHostRpc: async ({ method }) => {
        calls.push(method);
        if (method === "validateWorkflow") return { name: "Refactor" };
        if (method === "prepare") return { workspacePath: "/tmp/fabro-workflow", workflowVersionId: "a".repeat(64), baseSha: "a".repeat(40) };
        ...
        if (method === "evidence") return { run: { id: "fabro-1" }, events: [], artifacts: [], diff: { files: [] }, workspace: { path: "/tmp/fabro-workflow", files: {} }, errors: [] };
        throw new Error(`unexpected host method ${method}`);
      },
    });
    hosts.push(host); await plugin(host.bb);
    await host.harness.behavior.callAgentTool("fabro_workflow_submit", { requestKey: "lifecycle", workOrder }, { threadId: "thread-1", projectId: "project-1" });
