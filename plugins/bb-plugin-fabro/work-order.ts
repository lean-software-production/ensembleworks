...
export const workflowInputsSchema = z.record(z.string(), inputValue).refine(v => JSON.stringify(v).length <= 64_000, 'Workflow inputs exceed 64 KB');
...
export const submissionSchema = z.object({
  ...
  workOrder: workOrderSchema.omit({ setupCommands: true, validationCommands: true, qualityCommand: true }).extend({ workflow: workflowPathSchema, inputs: workflowInputsSchema.default({}) }),
}).strict();
