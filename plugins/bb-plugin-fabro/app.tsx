...
function title(job: Job) { return job.workOrder.title ?? job.workOrder.objective ?? "Fabro workflow"; }
...
function Directive({ attributes, message }: PluginMessageDirectiveProps) {
  ...
  if (!id) return <p className="rounded border border-destructive/40 p-2 text-xs text-destructive">Fabro workflow directive has no job id.</p>;
  if (error) return <p className="rounded border border-destructive/40 p-2 text-xs text-destructive">Could not load Fabro workflow: {error}</p>;
  if (!loaded) return <p className="rounded border border-border p-3 text-sm text-muted-foreground" role="status">Loading Fabro workflow…</p>;
  if (!job) return <p className="rounded border border-border p-3 text-sm text-muted-foreground">Fabro workflow not found.</p>;
  return <div className="my-2 max-w-md"><Card job={job} open={() => navigate.openThreadPanel({ actionId: ACTION, params: { jobId: id, ...(job.threadId !== message.threadId ? { originThreadId: job.threadId } : {}) }, title: "Fabro workflow" })} />{DONE.has(job.observationState) ? <button type="button" className="mt-1 px-1 text-xs text-primary hover:underline" onClick={() => navigate.toThread(job.threadId)}>Return for acceptance</button> : null}</div>;
}
...
function JobsPage() {
  ...
  return <div className="h-full overflow-y-auto p-4"><div className="mx-auto max-w-3xl space-y-3"><h2 className="text-lg font-semibold">Fabro</h2><p className="text-sm text-muted-foreground">Live Fabro runs started from BB threads.</p>{!threadId ? <p className="rounded-lg border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Open a work thread to inspect its Fabro workflows.</p> : jobs === null ? <p className="text-sm text-muted-foreground">Loading runs…</p> : jobs.length === 0 ? <p className="rounded-lg border border-dashed border-border p-6 text-center text-sm text-muted-foreground">No Fabro workflows yet. Submit work from an agent thread.</p> : jobs.map(job => <Card key={job.id} job={job} open={() => navigate.toThread(job.threadId)} />)}</div></div>;
}
