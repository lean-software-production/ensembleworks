...
describe("Fabro app", () => {
  it('shows a referenced run in another thread and keeps its owner in the sidebar', async () => {
    ...
    const slot = renderSlot(app.messageDirectives[0]!, {
      attributes: { jobId: job.id, originThreadId: job.threadId }, source: '::fabro-workflow',
    ...
    fireEvent.click(await slot.findByRole('button', { name: 'Open Fabro workflow Refactor parser' }));
    expect(slot.inspection.navigateCalls).toContainEqual({ method: 'openThreadPanel', options: {
      actionId: 'job', params: { jobId: job.id, originThreadId: job.threadId }, title: 'Fabro workflow',
    } });
  ...
  it("loads a live card and opens the thread panel with its job id", async () => {
    const app = await loadPluginApp(() => import("./app"));
    const slot = renderSlot(app.messageDirectives[0]!, {
      attributes: { jobId: job.id }, source: `::fabro-workflow{jobId="${job.id}"}`,
    ...
    fireEvent.click(slot.getByRole("button", { name: "Open Fabro workflow Refactor parser" }));
    expect(slot.inspection.navigateCalls).toContainEqual({
      method: "openThreadPanel", options: { actionId: "job", params: { jobId: "job-1" }, title: "Fabro workflow" },
    });
  });
  ...
  it("renders an RPC failure in the directive card", async () => {
    const app = await loadPluginApp(() => import("./app"));
    const slot = renderSlot(app.messageDirectives[0]!, {
      attributes: { jobId: job.id }, source: "::fabro-workflow", message: {
        id: "message-1", threadId: job.threadId, turnId: null, projectId: null,
      }, openWorkspaceFile: null,
    }, { rpc: { getJob: () => { throw new Error("job unavailable"); } } });
    await waitFor(() => expect(slot.getByText("Could not load Fabro workflow: job unavailable")).toBeTruthy());
  });
