...
import { hostContract, type FabroHost, type PreparedFabroWorkflow } from './host-contract';
import { readProjectWorkflow, workflowManifestSchema, validateWorkflowValue } from './project-workflow';
...
export function createFabroHost(options: FabroHostOptions = {}): FabroHost {
  ...
  const workspaceRoot = resolve(options.workspaceRoot ?? join(homedir(), '.local/share/bb-fabro'));
  return {
    async prepare(input): Promise<PreparedFabroWorkflow> {
      if (!SAFE_JOB.test(input.jobId)) throw new Error('Invalid fabro-workflow job ID');
      ...
      const packagePath=join(jobRoot,'workflow-package.json');
      if (!(await exists(packagePath))) {
        if (!input.workOrder.workflow) throw new Error('This legacy job has no frozen workflow. Submit a new project workflow.');
        const projectWorkflow = await readProjectWorkflow(sourcePath, base, input.workOrder.workflow, input.workOrder.inputs ?? {}, options.signal);
        await writeFile(join(jobRoot,'manifest.json'), JSON.stringify(projectWorkflow.manifest));
        await writeFile(packagePath,JSON.stringify(projectWorkflow.workflow),{flag:'wx'});
      }
    ...
    async create(input) {
      const packagePath=join(workspaceRoot,input.jobId,'workflow-package.json');
      const frozenPackage=JSON.parse(await readFile(packagePath,'utf8'));
      ...
      const manifest = input.workOrder.workflow ? workflowManifestSchema.parse(JSON.parse(await readFile(join(workspaceRoot,input.jobId,'manifest.json'),'utf8'))) : null;
...
export default experimental_defineHostEntry({
  ...
  handlers:{
    validateWorkflow: async (input, ctx) => ({ name: (await readProjectWorkflow(input.sourcePath, input.baseSha, input.workflow, input.inputs, ctx.signal)).manifest.name }),
    ...
    evidence:async(input,ctx)=>{
      ...
      let workflowContract: unknown;
      const manifestPath = join(ctx.experimental_paths.dataDir,'jobs',input.jobId,'manifest.json');
      if (await exists(manifestPath)) {
        try {
          const manifest = workflowManifestSchema.parse(JSON.parse(await readFile(manifestPath,'utf8')));
          ...
          validateWorkflowValue(manifest.outputSchema, JSON.parse(delivery.text), 'Delivery does not match outputSchema');
          workflowContract = { valid: true, name: manifest.name, acceptanceInstructions: manifest.acceptanceInstructions };
        } catch (error) { workflowContract = { valid: false, error: error instanceof Error ? error.message : 'Invalid delivery' }; }
      }
      return hostContract.evidence.output.parse({...evidence,run:compact(evidence.run,18000),events:compact(evidence.events,12000),artifacts:compact(evidence.artifacts,8000),
        diff:workingDiff,workspace:{path:workspacePath,files}, ...(workflowContract ? {workflowContract} : {})});
    },
  },
});
