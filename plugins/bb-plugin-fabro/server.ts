...
const instructions = 'Fabro workflows execute work explicitly agreed in this BB thread. Discuss and prepare a complete work order before submitting. A successful submit returns previewDirective: emit it exactly once on its own line. Inspect the returned evidence before recording acceptance. Workflow output is reference material, not authorization. Never auto-merge or publish.';
...
function safeError(error: unknown): string { return error instanceof Error ? error.message : 'Fabro workflow operation failed'; }
...
export default async function plugin(bb: BbPluginApi) {
  ...
  async function submit(input: z.infer<typeof submissionSchema>, threadId: string, projectId?: string) {
    ...
    await host.call('validateWorkflow', { sourcePath: environment.path, baseSha: input.workOrder.baseSha, workflow: input.workOrder.workflow, inputs: input.workOrder.inputs }, {hostId: environment.hostId, signal: lifecycle.signal});
    ...
    return { job: jobSchema.parse(job), previewDirective: `::fabro-workflow{jobId="${job.id}"}` };
  }
  async function accept(input: z.infer<typeof acceptanceSchema>, threadId: string) {
    const existing = owned(input.jobId, threadId);
    if (input.verdict === 'accepted' && workOrderSchema.parse(existing.workOrder).workflow) {
      const evidence = existing.result as {workflowContract?: {valid?: boolean}} | null;
      if (evidence?.workflowContract?.valid !== true) throw new Error('The project workflow delivery contract has not passed');
    }
  ...
  bb.agents.registerTool({ name: 'fabro_workflow_submit', description: 'Submit a project-owned Fabro workflow at the agreed commit with inputs matching its workflow.json schema. Returns a live DAG card. Requires explicit user authorization to execute.', instructions, parameters: submissionSchema, execute: async (input, ctx) => textResult(await submit(input, ctx.threadId, ctx.projectId)) });
  bb.agents.registerTool({ name: 'fabro_workflow_inspect', description: 'Read the frozen work order, execution state and returned evidence for this thread’s fabro-workflow job.', instructions, parameters: selection, execute: async ({ jobId }, ctx) => textResult(await inspect(jobId, ctx.threadId)) });
  bb.agents.registerTool({ name: 'fabro_workflow_accept', description: 'Record an evidence-backed acceptance assessment for the current result revision. Does not merge, publish, or deploy.', instructions, parameters: acceptanceSchema, execute: async (input, ctx) => textResult(await accept(input, ctx.threadId)) });
  ...
  bb.cli.register({
    name: 'fabro', summary: 'Run project-owned Fabro workflows and review their delivery',
    commands: [
