...
// like `snapshot()`.
import { orderForPaint, queryViewport, type Bounds, type Shape } from '@ensembleworks/canvas-model'
...
export function ShapeLayer({ toolContext, camera, viewportSize, dispatch }: ShapeLayerProps) {
  ...
  // DFS pre-order over this CULLED (visible) set: each level's siblings —
  // starting with this set's own forest roots (a shape whose parent fell
  // outside the culled set is treated as a root, never dropped) — sorted by
  // `(index ASC, id ASC)`, then recursively each shape's in-set children.
  // This subsumes the old parent-before-child guarantee (a parent always
  // precedes its descendants in DFS pre-order, so an opaque container like
  // FrameShape's fully-opaque body can never occlude its own children) AND
  // additionally makes the doc's `index` field the visible z-order: a
  // higher-index shape paints later (on top) among its siblings, and the
  // `(index, id)` tie-break gives every peer the SAME deterministic order
  // for the all-'a1' legacy corpus (no data migration performed — see the
  // z-order plan's Decision D-2) regardless of queryViewport's iteration
  // order, which still carries no correctness meaning here.
  const visibleShapes = orderForPaint(
    visibleIds
