...
// Run: bun src/web-viewer/followLogic.test.ts
...
const p = presenterFor([peer('a', { webViewerPresent: { shapeId: 'shape:1', fraction: 0.5 } })], 'shape:1')
...
assert.equal(
	presenterFor([peer('a', { webViewerPresent: { shapeId: 'shape:1', fraction: 0.5 } }, '#FF802B')], 'shape:1')
		?.color,
	'#FF802B'
)
assert.equal(presenterFor([peer('a', { webViewerPresent: { shapeId: 'shape:2', fraction: 0.5 } })], 'shape:1'), null)
assert.equal(presenterFor([peer('a', { webViewerPresent: { shapeId: 42 } })], 'shape:1'), null)
...
const two = [
	peer('a', { webViewerPresent: { shapeId: 'shape:1', fraction: 0.1, ts: 100 } }),
	peer('b', { webViewerPresent: { shapeId: 'shape:1', fraction: 0.9, ts: 200 } }),
]
...
const stale = [
	peer('a', { webViewerPresent: { shapeId: 'shape:1', fraction: 0.1 } }),
	peer('b', { webViewerPresent: { shapeId: 'shape:1', fraction: 0.9, ts: 5 } }),
]
...
// presenter can detect a steal (peer ts newer than their own token's).
{
	const winner = presenterFor(
		[
			peer('a', { webViewerPresent: { shapeId: 's1', fraction: 0.1, ts: 100 } }),
			peer('b', { webViewerPresent: { shapeId: 's1', fraction: 0.9, ts: 200 } }),
		],
		's1'
	)
...
{
	const winner = presenterFor(
		[peer('a', { webViewerPresent: { shapeId: 's1', fraction: 0.1 } })],
		's1'
	)
