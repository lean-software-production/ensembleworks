...
assert.equal(
	...
	'wss://canvas.example.com/api/terminal/ws?session=abc1&cols=80&rows=24'
)

// Gateway set → relay path under /api (prod Caddy routes /api/terminal/* elsewhere).
assert.equal(
	buildTermWsUrl(loc, 'abc1', 80, 24, 'gw-box'),
	'wss://canvas.example.com/api/terminal/relay?session=abc1&gateway=gw-box&cols=80&rows=24'
)

// http origin → ws scheme.
assert.equal(
	buildTermWsUrl({ protocol: 'http:', host: 'localhost:5173' }, 'x', 10, 5, 'g'),
	'ws://localhost:5173/api/terminal/relay?session=x&gateway=g&cols=10&rows=5'
)
