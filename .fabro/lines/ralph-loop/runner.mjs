...
function policy() {
  ...
  const inScope = changed.every(file => file !== order.planPath && !file.startsWith('.fabro/lines/') && order.scope.some(raw => {
...
try {
  ...
  if (phase === 'baseline') {
    ...
    if (scope.changed.length) {
      ...
      if (hasStaged.status === 1) execFileSync('git',['-c','user.name=Assembly Lines','-c','user.email=assembly-lines@localhost','commit','-m','feat: implement approved plan'],{stdio:'inherit'});
