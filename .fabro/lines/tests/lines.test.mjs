...
function fixture(t, name, {inputs={},scope=['src'],attempts=2}={}) {
  ...
  mkdirSync(join(cwd,'.fabro/lines'),{recursive:true}); cpSync(join(lines,name),join(cwd,'.fabro/lines',name),{recursive:true});
  ...
  const run = phase => spawnSync(process.execPath,[`.fabro/lines/${name}/runner.mjs`,phase],{cwd,encoding:'utf8'});
