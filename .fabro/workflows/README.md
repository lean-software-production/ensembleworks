...
described in each workflow README; plugin checks do not cover those surfaces.
...
plus its reproduction command. Commit the workflow definitions before submission.
...
frozen workflow package. The general refactor now uses explicit work-order commands;
...
Verify project Ralph guards with `node --test .fabro/workflows/tests/workflows.test.mjs`.
Set `FABRO_GRAPH_TEST=1` for its optional local-server shell-only graph regression.
Run `npm --prefix plugins/bb-plugin-fabro run test:recipes` for the
