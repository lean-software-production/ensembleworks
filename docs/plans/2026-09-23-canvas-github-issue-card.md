...
The user approved a read-only, plain-text preview of cached issue descriptions. The existing `canvas_github_repo` reply stays body-free for already-open clients. `canvas_github_repo_v2` projects a 4096-character Markdown-to-text excerpt from the installed GitHub plugin's cache, after the same project/repository authorization check. No issue body enters shape props or Loro. The issue-number link opens the complete description on GitHub.
...
The user then preferred a clipping area over calculated line counts. The card now mounts the same preview text at every size in a flexible `overflow: hidden` div. The title, status, labels, and footer retain their space; the div grows into whatever remains. The revised interaction contract failed against the old line-clamped renderer with `AssertionError: expected 'the same description must stay mounte…' to be null`. Browser checks confirm that the clipping area grows with the card while its text stays constant.
